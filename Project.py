...
app.layout = html.Div(id='main-div', children=[
    ...
    # Running Processes Table (moved to the top)
    html.H3("Running Processes", style={'color': '#FFD700', 'marginTop': '20px', 'textAlign': 'center'}),
    html.Div([
    html.Button("More", id="more-button", n_clicks=0, style={'padding': '10px', 'backgroundColor': '#444', 'color': 'white'})
], 
style={'display': 'flex', 'justifyContent': 'center', 'marginTop': '10px'}),
    html.Div(id='process-table-container'),
    
    # Graph containers for system usage
    ...
    # Additional Graphs Section (Pie Chart, Histogram, etc.)
    html.Div(style={'display': 'flex', 'justifyContent': 'space-around', 'flexWrap': 'wrap', 'marginTop': '20px'}, children=[
        # Pie Chart
        html.Div([
...
def update_dashboard(n, more_clicks, theme_button_clicks, kill_clicks):
    ...
    for proc in processes[:limit]:
        proc_info = [
            ...
            html.Button('End Task', id=f"kill-{proc['pid']}", n_clicks=0, style={'backgroundColor': 'red', 'color': 'white', 'padding': '5px', 'border': 'none'})
        ]
    ...
    bar_fig = go.Figure(data=[go.Bar(x=['Disk', 'Network'], y=[disk_usage, net_usage / (1024 * 1024)], marker=dict(color='#1E90FF'))])
    bar_fig.update_layout(title="Disk vs Network Usage", xaxis_title="Resource", yaxis_title="Usage", plot_bgcolor=background_color, paper_bgcolor=background_color, font=dict(color=text_color))
